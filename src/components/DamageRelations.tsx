...
const DamageRelations = ({damages}: DamageModalProps) => {
  ...
  const [damagePokemonForm, setDamagePokemonForm] = useState<SeparateDamageRelations>();
  ...
  const filterDamageRelations = (valueFilter: string, damage: DamageRelationsProps) => {
    const result: SeparateDamageRelations = Object.entries(damage).filter(([keyName, _]) => {
